...
static int update(UPDATE_FUNC_ARGS)
{
	...
	if (temp > 320.0f && sim->pv.at(x/CELL, y/CELL) > 50.0f && sim->rng.chance(1, 150))
	{
		//@ SEED -> MWAX
	...
	if (water > 3)
	{
		...
		if (TYP(pmap.at(x+dir3x3[down].X, y+dir3x3[down].Y)) == PT_SAND && !TYP(pmap.at(x+dir3x3[up].X, y+dir3x3[up].Y)))
		{
			if (parts[i].life > 200)
			{
				// Start growth
